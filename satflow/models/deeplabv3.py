...
class DeeplabV3(pl.LightningModule):
    def __init__(
        ...
        if backbone in ["r101", "resnet101"]:
            self.model = deeplabv3_resnet101(pretrained=pretrained, num_classes=forecast_steps)
        else:
            self.model = deeplabv3_resnet50(pretrained=pretrained, num_classes=forecast_steps)
