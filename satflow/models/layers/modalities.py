...
def attend(q, k, v, dropout_prob=0.0, attention_mask=None):
    ...
    if attention_mask is not None:
        ...
        large_k = jnp.array(1e4 if attention.dtype == jnp.float16 else 1e30, dtype=attention.dtype)
    ...
    if attention_mask is not None:
        ...
        # zero. We force zeros.
        wipe_attn = jnp.all(attention_mask == 0, axis=2, keepdims=True)  # shape (B, T, 1)
...
class Attention(hk.Module):
    ...
    def __call__(self, inputs_q, inputs_kv, attention_mask=None):
        ...
        result = attend(q, k, v, dropout_prob=self._dropout_prob, attention_mask=attention_mask)
        return conv_1d(
...
class MLP(hk.Module):
    ...
    def __call__(self, x, *, is_training):
        ...
        x = conv_1d(
            output_channels=self._widening_factor * output_channels, init_scale=self._init_scale
        )(x)
        x = jax.nn.gelu(x)
...
class CrossAttention(hk.Module):
    ...
    def __call__(self, inputs_q, inputs_kv, *, attention_mask=None, is_training):
        ...
        if self._shape_for_attn == "q":
            ...
            raise ValueError(f"Unknown value {self._shape_for_attn} for " "shape_for_attention.")
