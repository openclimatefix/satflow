...
class SatFlowDataset(thd.IterableDataset, wds.Shorthands, wds.Composable):
    ...
    def __init__(self, datasets, config, train=True):
        ...
        self.forecast_times = config.get("forecast_times", 48) # Max timesteps to predict ahead (minutes / 5) default 4 hours

        # Defined output sizes, etc.
        self.output_shape = config['output_shape']
        self.target_type = config.get("target", "cloudmask")
        # Should load the common data here
        self.bands = config.get("bands", ('HRV', 'IR016', 'IR039', 'IR087', 'IR097', 'IR108', 'IR120', 'IR134', 'VIS006', 'VIS008', 'WV062', 'WV073'))
    ...
    def get_timestep(self, sample, idx):
        ...
        if self.use_time:
            image = np.concatenate([image, create_time_layer(sample['time.pyd'][idx], shape=(self.output_shape, self.output_shape))], axis=-1)
        if self.use_mask:
            image = np.concatenate([image, np.expand_dims(binarize_mask(sample[f'cloudmask.{idx:03d}.npy']), axis=-1)], axis=-1)
