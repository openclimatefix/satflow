...
for root, dirs, files in os.walk(eumetsat_dir):
    ...
    if files:
        ...
        if prev_datetime.date() != datetime_object.date():
            ...
            sink_flow.close()
            sink = wds.TarWriter(
                f"/run/media/bieker/data/EUMETSAT/satflow-{shard_num:05d}.tar",
                compress=True,
            )
            sink_flow = wds.TarWriter(
                f"/run/media/bieker/data/EUMETSAT/satflow-flow-{shard_num:05d}.tar",
                compress=True,
            )
            ...
            logger.debug(
                f"On shard: {shard_num} Date: {datetime_object.strftime('%Y/%m/%d/%H/%M')}"
            )
            shard_num += 1
